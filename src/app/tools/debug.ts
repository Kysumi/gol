...
const showMouseLocation = (app : pixi.Application, renderer : pixi.Renderer) => {    
    ...
    if(typeof mouse == 'undefined'){
        ...
        girdLocation = worldPositionToGrid(layout,Point(Math.round(mouse.x),Math.round(mouse.y)))
    }
    const txt = new pixi.Text(`Mouse: ${mouse.x.toFixed(2)}, ${mouse.y.toFixed(2)}`, {
        fill: "#800080",
        fontSize: 12,
        fontWeight: "bold"        
    });
...
export const drawDebug = (app : pixi.Application, ticker : pixi.Ticker, renderer : pixi.Renderer) => {
